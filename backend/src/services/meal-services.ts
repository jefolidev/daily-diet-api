import {
  ...
  putMealById,
  selectAllMeals,
} from '../database/queries/meals-queries'
...
export async function getAllTheMeals() {
  try {
    ...
    console.error(
      'An error occurred while trying to get the meals. See the error below: ',
      error,
    )
...
export async function createNewMeal(mealData: MealType, accountId: string) {
  try {
    ...
    console.error(
      'An error occurred while trying to create a meal. See the error below: ',
      error,
    )
...
export async function updateMealById(
  updatedMeal: Omit<MealType, 'user_id'>,
  mealId: string,
) {
  try {
    if (!mealId) {
    ...
    console.error(
      'An error occurred while trying to update a meal. See the error below: ',
      error,
    )
...
export async function removeMealById(mealId: string) {
  try {
    ...
    console.error(
      'An error occurred while trying to remove a meal. See the error below: ',
      error,
    )
